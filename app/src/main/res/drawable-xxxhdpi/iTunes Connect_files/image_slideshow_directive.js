	...
	itcApp.directive('imageSlideshow',function($timeout, $sce){
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                scope.setDimensionsOld = function() {
                    var maxHeight=0;
                    var maxWidth=0; 
                    var image;

                    var existingImages = scope.getRightImageSet();
                    ...
                    if (!scope.showWatchImages && !scope.hasVideoError() && scope.videoWidth && scope.videoHeight) {
                ...
                scope.onFirstSlide = function() {
                    if (scope.show) {
                        var videoIsPrevious = !scope.showWatchImages && scope.currentIndex === 0 && scope.videos.length>0 && !scope.hasVideoError();
                ...
                scope.onLastSlide = function() {
                    var existingImages = scope.getRightImageSet();
                    if (scope.show) {
                        if (!scope.showWatchImages && scope.videoShowing) { // if on a video, we're only on the last slide if there are no images
                            return existingImages.length === 0; 
                        }
                        else {
                            return (scope.currentIndex === (existingImages.length-1));
                        }
                    }
                    else {
                        return true;
                ...
                scope.slideLeft = function() {
                    ...
                    $timeout(function() {
                        if (!scope.showWatchImages && scope.videoShowing) { // if on video, go to last image
                            scope.currentIndex = existingImages.length-1;
                            scope.videoShowing = false;
                            scope.stopVideo();
                        }
                        else if (!scope.showWatchImages && scope.currentIndex === 0 && scope.videos.length>0 && !scope.hasVideoError()) { // if on first image, go to video, if there is a video
                ...
                scope.slideRight = function() {
                    ...
                    $timeout(function() {
                        if (!scope.showWatchImages && scope.videoShowing) { // if on video, go to first image
                            scope.currentIndex = 0;
                            scope.videoShowing = false;
                            scope.stopVideo();
                        }
                        else if (!scope.showWatchImages && scope.currentIndex === (existingImages.length-1) && scope.videos.length>0 && !scope.hasVideoError()) { // if on last image, go to video, if there is a video
                            scope.videoShowing = true;
                        }
                        else if (scope.currentIndex === (existingImages.length-1)) { // if on last image, but no video, go to first image
                            scope.currentIndex = 0;
                            scope.videoShowing = false;
                        }
                ...
                scope.getRightImageSet = function() {
                    var images;
                    if (scope.showWatchImages) {
                        images = scope.watchImages;
                    }
                    else {
                        images = scope.images;
                    }
                ...
                scope.getImgSrc = function() {
                    ...
                    if (scope.currentIndex !== -1 && existingImages && (scope.currentIndex<existingImages.length)) {
                        var image = existingImages[scope.currentIndex];
                        var url = image.data;
                        return url;
                    }
                };
