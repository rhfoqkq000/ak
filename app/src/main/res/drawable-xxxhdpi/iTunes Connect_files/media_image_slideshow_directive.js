define(['sbl!app'], function (itcApp) {
	...
	itcApp.directive('mediaImageSlideshow',function($timeout, $sce){
        /* for ss */
        return {
            restrict: 'A',
            ...
            link: function(scope, element, attrs) {
                ...
                scope.onFirstSlide = function() {
                    if (scope.show) {
                        var videoIsPrevious = scope.videos && scope.currentIndex === 0 && scope.videos.length>0 && !scope.hasVideoError();
                ...
                scope.slideLeft = function() {
                    ...
                    $timeout(function() {
                        if (scope.videos && scope.videoShowing) { // if on video, go to last image
                            scope.currentIndex = scope.images.length-1;
                            scope.videoShowing = false;
                            scope.stopVideo();
                        }
                        else if (scope.videos && scope.currentIndex === 0 && scope.videos.length>0 && !scope.hasVideoError()) { // if on first image, go to video, if there is a video
                            //scope.currentIndex = scope.images.length-1;
                ...
                scope.slideRight = function() {
                    ...
                    $timeout(function() {
                        if (scope.videos && scope.videoShowing) { // if on video, go to first image
                            scope.currentIndex = 0;
                            scope.videoShowing = false;
                            scope.stopVideo();
                        }
                        else if (scope.videos && scope.currentIndex === (scope.images.length-1) && scope.videos.length>0 && !scope.hasVideoError()) { // if on last image, go to video, if there is a video
                            scope.videoShowing = true;
                        }
